...
if [ "$1" != "" ]; then
  ...
  exec sudo -u www-data php ${project_dir}/artisan artisan octane:start --server=swoole --host=0.0.0.0 --port=9000 --workers=auto --task-workers=auto --max-requests=500
elif [ ${container_mode} = "horizon" ]; then
  exec sudo -u www-data php ${project_dir}/artisan horizon
elif [ ${container_mode} = "scheduler" ]; then
  echo "*/1 * * * * php ${project_dir}/artisan schedule:run --verbose --no-interaction" > /etc/supercronic/laravel
  exec sudo -u www-data supercronic /etc/supercronic/laravel
else
